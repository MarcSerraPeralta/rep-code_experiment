...
CLASSIFIER = DecayLinearClassifier
...
for element in sequence_generator(STRING_DATA):
    ...
    for qubit in iq_readout_data.qubit:
        ...
        for format_ in ["pdf", "png"]:
            fig.savefig(
                cal_dir / f"2state_{cla_name}_{qubit_name}_no-ps.{format_}",
                format=format_,
            )
        plt.close()
        ...
        # plot readout calibration without PS
        fig, ax = plt.subplots()
        fig = summary(classifier, shots_0_ps, shots_1_ps)
        for format_ in ["pdf", "png"]:
            fig.savefig(
                cal_dir / f"2state_{cla_name}_{qubit_name}_ps.{format_}",
                format=format_,
            )
        plt.close()

    np.save(cal_dir / f"2state_{cla_name}_params_no-ps.npy", calibration_params_no_ps)
    np.save(cal_dir / f"2state_{cla_name}_params_ps.npy", calibration_params_ps)
